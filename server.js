...
function sendVerificationEmail(email, code, type) {
    const messages = {
        'signup': `Welcome to MY SCHOOL BW!\n\nYour verification code is: ${code}\n\nEnter this code to verify your account.\n\nThis code expires in 10 minutes.`,
        'login': `Login Verification\n\nYour verification code is: ${code}\n\nEnter this code to complete your login.\n\nThis code expires in 10 minutes.`,
        'reset': `Password Reset\n\nYour password reset code is: ${code}\n\nEnter this code to reset your password.\n\nThis code expires in 10 minutes.`
    };
    
    console.log(`📧 [${type.toUpperCase()}] Code for ${email}: ${code}`);
    console.log(`📧 Message: ${messages[type] || 'Verification code'}`);
    
    // In production, use real email service here
    // await sendEmail(email, `Your ${type} code`, messages[type]);
...
function generateToken(user) {
    return jwt.sign(
        { id: user.id, email: user.email, schoolId: user.schoolId, role: user.role || 'parent' },
        JWT_SECRET,
        { expiresIn: '7d' }
    );
}
...
app.post('/api/signup', async (req, res) => {
    try {
        const { username, email, password, phone, schoolId, accountType } = req.body;
        ...
        const newUser = {
            ...
            phone: phone || '',
            schoolId,
            role: accountType === 'school' ? 'admin' : 'parent',
            isSchoolAdmin: accountType === 'school' ? true : false,
            isVerified: false,
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (!user.isVerified) {
            ...
            sendVerificationEmail(email, code, 'login');
...
app.post('/api/verify', async (req, res) => {
    try {
        ...
        const index = verifications.findIndex(v => v.id === record.id);
...
app.post('/api/users/check', (req, res) => {
    try {
        ...
        if (user) {
            ...
            res.json({ success: false, exists: false, message: 'No account found with this email.' });
        }
    } catch (error) {
...
app.get('/api/students/guardian/:email', authenticate, (req, res) => {
...
// Register child
app.post('/api/register-child', authenticate, (req, res) => {
    const students = readData('students.json');
...
app.get('/api/reports/student/:studentId', authenticate, (req, res) => {
...
app.get('/api/test-results/child/:childId', authenticate, (req, res) => {
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📧 Email Verification: Signup, Login, Forgot Password`);
